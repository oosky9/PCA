...
def process_train(args, x_train):
    ...
    pca_mean      = pca.get_mean_vector()
    pca_eigen_val = pca.get_eigen_value()
    pca_eigen_vec = pca.get_eigen_vector()

    pca_cr  = pca.get_contribution_ratio()
    pca_ccr = pca.get_cumulative_contribution_ratio()
...
def process_test(args, x_test):
    ...
    pca.set_mean_vector(mean_vect)
    pca.set_eigen_value(eigen_value)
    pca.set_eigen_vector(eigen_vect)

    projected     = pca.process_projection(x_test)
    reconstructed = pca.process_inverse(projected)
...
def process_arbit(args, x_train):
    ...
    projected = pca.process_projection(x_train)
    ...
    for i in range(args.n_shape):
        arb = pca.process_inverse(z_sample[i])
        arbit.append(arb)
